...
function(e) {
	...
	x.prototype.events = function() {
		...
		e.elem.on("dblclick", 'input[lay-filter="layTransferCheckbox"]+',
		function() {
			clearTimeout(times);
			a(this).prev().prop('checked',true);
			var i = a(this).parent().parent().parent().data("index");
			a(this).parents().find('button[data-index="'+i+'"]').removeClass(o);
			e.layBtn.click();
		}),
		e.layBtn.on("click",
		function() {
		...
		e.laySearch.find("input").on("keypress",function(event) {
			if(event.keyCode == "13")    
            {
				return false;
            }
