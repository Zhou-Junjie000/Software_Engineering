...
layui.define(['element', 'table', 'layer', 'form', 'jquery', 'laytpl'], function (exports) { // 提示：模块也可以依赖其它模块，如：layui.define('layer',
    ...
    var contractmyTable = {
         ...
         */
        /*render(options) {
            contractmyTable.tableIns = table.render(Object.assign({}, DEFAULT_TABLE_CONFIG, options));
            if(!GLOBAL_TABLE)GLOBAL_TABLE=contractmyTable.tableIns;
            return contractmyTable;
        },*/
        ...
        render(options, tableUrls) {
        	contractmyTable.tableIns = table.render(Object.assign({}, DEFAULT_TABLE_CONFIG, options));
        	contractmyTable.tableUrls = tableUrls;
            if(!GLOBAL_TABLE)GLOBAL_TABLE=contractmyTable.tableIns;
            return contractmyTable;
        },
         ...
         */
        query(options) {
            contractmyTable.backWhere = Object.assign({}, options);
            contractmyTable.tableIns.reload({
            	url: contractmyTable.tableUrls["list"].url,
                where: contractmyTable.backWhere
                , page: {
                    curr: 1 // 重新从第 1 页开始
                }
                ,done: function (res, curr, count) {
                 	...
                 	res.data.forEach(function (item, index) {//排序后渲染效果
                 		if (item.overTime) {
                 		 ...
                 		 if($('#reportuserworkloadValue').length > 0){
                 			v1 = parseFloat(v1)+parseFloat(!item.reportuserworkload?0:item.reportuserworkload) 
                			// v2 = parseFloat(v2)+parseFloat(!item.estimateworkload?0:item.estimateworkload)
                			v2 = parseFloat(v2)+parseFloat(!item.contractworkload?0:item.contractworkload)
                			v3 = parseFloat(v3)+parseFloat(!item.allactualday?0:item.allactualday);
                 		 }
                 	});
                 	$('#reportuserworkloadValue').html(v1+"");
            		$('#estimateworkloadValue').html(v2+"");
            		$('#actualdayValue').html(v3+"");
                    //无数据时显示无数据相关提示
                    if (res.data.length === 0) {
         ...
         */
        refresh(where) {
            contractmyTable.query(contractmyTable.backWhere);
        },
            ...
            layer.confirm(
                ...
                function (index) {
                    ...
                    $.post(`${SYS_CFG.ctxPath}/${Entity.tag}/del`, {
            	        ...
            	        var where = Object.assign({"groupBy":contractmyTable.tableUrls[tableName].groupBy}, contractmyTable.backWhere);
            	        table.reload(tableName,{
            	            url: contractmyTable.tableUrls[tableName].url,
            	            where: where, 
            	            page: {
            	                curr: 1 // 重新从第 1 页开始
            	            }
	...
	contractmyTable.getSort(`${Entity.tag}`);
	/**
	 * 自定义排序事件
	 */
	 $(document).on('click', '.sort-btn', function () {
		 ...
		 if(isSort){
			 ...
			 contractmyTable.backWhere.sort=tableSort[index];
			 contractmyTable.backWhere.order=" ";
		     contractmyTable.refresh(contractmyTable.backWhere);
		 }else{
			 if(defaultSortWhere!=null){
				 contractmyTable.backWhere.sort=defaultSortWhere;
			 }else{
				 contractmyTable.backWhere.sort="";
			 }
			 contractmyTable.backWhere.order=" ";
			 contractmyTable.refresh(contractmyTable.backWhere);
		 }
    ...
    // $(document).on('click', '.my-table-refresh', function () {
    //     debugger;
    //      var queryParams =contractmyTable.backWhere;
    //     $("input[type=checkbox][name=1]").each(function(){
    //     	var $this = $(this);
    //     	if(!!$this.prop("checked")/*&&$(this).parent().parent().hasClass('layui-this')*/){
    //     		if($this.attr("tablename") == "list"){
    //     			contractmyTable.refresh(queryParams);
    //     		}else{
    //     			var tableName = $this.attr("tablename");
    //     	        var where = Object.assign({"groupBy":contractmyTable.tableUrls[tableName].groupBy}, queryParams);
    //     	        table.reload(tableName,{
    //     	            url: contractmyTable.tableUrls[tableName].url,
    //     	            where: where,
    //     	            page: {
    ...
    $(document).on('change', '#fastday', function () {
        ...
        contractmyTable.query(queryParams);
    });
...
//    });
    ...
    /*form.on("submit(advancedSearchSubmit)", function (data) {
    	debugger
        var queryParams = contractmyTable.advSearch(data);
        var value = $('.fastsearchInput').val();
        var name = $('.fastsearchInput').attr('name');
        queryParams[name] = value;
        contractmyTable.query(queryParams);
    ...
    form.on("submit(advancedSearchSubmit)", function (data) {
        ...
        var name = $('.fastsearchInput').attr('name');
        queryParams[name] = value;
        //因为查询和左侧树没关系，所有再次重新赋值
        if (!queryParams.leve) {
            queryParams['leve'] = null;
        }
        contractmyTable.initWhere = Object.assign({}, queryParams);
        var tableName = "list";
        var where = Object.assign({"groupBy":contractmyTable.tableUrls[tableName].groupBy}, queryParams, {typeid:""});
        table.reload(tableName,{
            url: contractmyTable.tableUrls[tableName].url,
            where: where, 
            page: {
    ...
    form.on("submit(advancedSearchReset)", function (data) {
    	...
    	$.each(contractmyTable.advSearch(data),function(key,values){
...
//    });
    ...
    function openWin(title, url, success, area) {
        ...
        return topLayer.open({
            ...
            yes: function (layero, index) {
                contractmyTable.refresh();
            },
            success: function (layero, index) {
            	if (!!layero.find("iframe")[0].contentWindow.initConfig) 
            		layero.find("iframe")[0].contentWindow.initConfig(contractmyTable);
                if (success) {
    ...
    contractmyTable.on('sort(list)', function (obj) {
        let options = {sort: obj.field, order: obj.type}
        contractmyTable.query(options);
        // resize();
        tableHeight();
		setTimeout(function () {
		   initTD($("#list"));
    ...
    // 输出test接口
    exports('contractmyTable', contractmyTable);
});
//是否是禅道嵌套
