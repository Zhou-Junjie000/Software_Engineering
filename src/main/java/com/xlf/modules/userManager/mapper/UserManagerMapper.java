...
<<<<<<< HEAD
		...
		"WHERE " + 
=======
		"SELECT id,safepassword, account, NAME, mail, dept, sex,( SELECT name FROM t_role WHERE id = roleid ) AS role "+
		"FROM " +
		"	t_user " +
		"WHERE " +
>>>>>>> e1a8a41 (sprint4)
...
=======
		"insert into t_user(account,password,name,mail,dept,sex,roleid,safepassword) "
		+ "values('${entity.account}','${entity.password}','${entity.name}',"
		+ "'${entity.mail}','${entity.dept}','${entity.sex}','${entity.roleid}','${entity.safepassword}')" +
>>>>>>> e1a8a41 (sprint4)
		"</script>"})
...
=======
	@Update({"<script>",
			"update t_user set safepassword = '${newPassword}' where id = '${id}'" +
					"</script>"})
	void updatePassword2(@Param("id")String id, @Param("newPassword")String newPassword);
>>>>>>> e1a8a41 (sprint4)
