...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
		...
		} }, { icon: 'xm-iconfont icon-qingkong1', name: '清空', click: function click(id, cm) {
	...
	Common.prototype.init = function (target) {
		...
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			...
			value = othis.find('option[selected]').toArray().map(function (option) {
			 ...
			 allOption = othis.find('option').toArray().map(function (option) {
					return {
						name: option.innerHTML,
						value: option.value
					};
	...
	Common.prototype.changePlaceHolder = function (div) {
		...
		if (data[id] && data[id].config.height) {//既然固定高度了, 那就看着办吧
			...
			title.css('height', (height > 36 ? height + 2 : height) + 'px');
			...
			if (label.is('.layui-form-label') && title.parents('.layui-form-pane')[0]) {
				height = height > 36 ? height + 2 : height;
	...
	Select4.prototype.alloption = function (id) {
		if (typeof id != 'string') {
			return [];
		}
		var fs = data[id];
